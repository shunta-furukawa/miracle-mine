...
X の @PeriTune_Info は 2026-09-18 時点でプロフィールが開けない（閉鎖または利用停止）ため、メンションせず peritune.com へリンクする。PeriTune さんの規約ではクレジットも利用報告も任意なので、この投稿は御礼であって義務ではない。「公認」「提供」と読める書き方は避ける（CC BY 4.0 はライセンサーによる推薦の示唆を禁じている）。

> ミラクルマインの音楽は、すべて PeriTune さんのフリー素材をお借りしています。森・海・洞窟・火山・空、章ごとに世界が変わるのは、この曲たちのおかげです。素敵な音楽をありがとうございます🎵
> https://peritune.com/
> 使用曲の一覧 https://miracle-mine.vercel.app/about
> #ミラクルマイン #PeriTune

画像: `docs/x-assets/post3-*.jpg` か `post4-*.jpg`（章の風景が写っているもの）。
