...
export function showDialogue(story,onDone){
 ...
 root.innerHTML=`<section class="story-scene"><div class="story-backdrop"></div><div class="story-wash"></div><header class="story-header"><h2></h2><button class="story-skip">スキップ ≫</button></header><div class="story-actor story-left" aria-hidden="true"></div><div class="story-actor story-right" aria-hidden="true"></div><div class="story-goal"></div><div class="story-box"><div class="story-speaker"></div><p class="story-words" aria-hidden="true"></p><p class="sr-only story-accessible" aria-live="polite" aria-atomic="true"></p><div class="story-footer"><span class="story-count"></span><button class="story-next">全文を表示 ▸</button></div></div></section>`;
 ...
 function sprite(el,n){el.style.backgroundPosition=`${n%4/3*100}% ${Math.floor(n/4)*100}%`;}
 ...
 const left=line.who==='ルカ';sprite(q('.story-left'),left?0:7);sprite(q('.story-right'),portraits[story.partner]);q('.story-left').classList.toggle('speaking',left);q('.story-right').classList.toggle('speaking',!left);paint();if(!reduced)timer=setInterval(()=>{visible=Math.min(chars.length,visible+1);paint();if(visible===chars.length)clearInterval(timer);},32);}
