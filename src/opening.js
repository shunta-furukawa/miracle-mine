...
let dismiss;
export function showOpening(onDone=()=>{},{onTransition=()=>{}}={}){
 ...
 function transition(fadeOut,fadeIn){if(transitioning)return;transitioning=true;onTransition({fadeOut:reduced?0:fadeOut,fadeIn:reduced?0:fadeIn});}
 function finish(run=true){if(closed)return;if(run)transition(.35,1.25);closed=true;cancelAnimationFrame(frame);pan?.cancel();document.removeEventListener('visibilitychange',visibility);root.close();root.remove();dismiss=null;if(previous?.isConnected)previous.focus({preventScroll:true});if(run)onDone();}
 ...
 if(elapsed>=total){transition(.85,2.1);veil.style.opacity=String(Math.min(1,(elapsed-total)/fadeDuration));root.classList.add('is-ending');}else paint();frame=requestAnimationFrame(tick);}
