...
export const openingShots=[
 ...
 {asset:'opening-workshop.webp',duration:4700,line:'いつか、この翼で。',label:'BEYOND THE CLOUDS'}
];
let dismiss;
export function showOpening(onDone=()=>{}){
 ...
 const total=openingShots.reduce((n,s)=>n+s.duration,0);
 ...
 function tick(now){if(closed)return;if(!paused&&!document.hidden)elapsed+=Math.min(100,Math.max(0,now-last));last=now;if(elapsed>=total){finish();return;}paint();frame=requestAnimationFrame(tick);}
