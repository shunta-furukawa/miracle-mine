...
export const freshSlot=()=>({cleared:[],paint:0,wing:0,propeller:0,decoration:0,failures:0,flightName:'',ranking:null,sky:voyageRecord(),updated:Date.now()});
...
export function normalize(value) {
 ...
 if(Array.isArray(value.slots))data.slots=data.slots.map((_,i)=>{
   ...
   result.flightName=flightName(s.flightName);result.ranking=identity(s.ranking);result.sky=voyageRecord(s.sky);
