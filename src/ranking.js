...
export class RankingClient{
 ...
 async begin(s,persist){
  ...
  const run={runId:crypto.randomUUID(),key,uid:registered.uid,events:[],count:0,distance:0,status:'flying',closed:false};
  await this.api('start',{runId:run.runId,protocol:1,name:flightName(s.flightName),design:flightDesign(s)},key);
