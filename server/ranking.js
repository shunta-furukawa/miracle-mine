...
import {voyageStage} from '../src/sky-voyage.js';
...
export function service(query){
 ...
 return async function dispatch(action,b={},key=''){
  if(action==='status')return {ready:true,protocol:1};
  if(action==='board'){const rows=await query('SELECT uid,distance,snapshot,achieved_at FROM mm_bests ORDER BY distance DESC,achieved_at ASC,uid ASC LIMIT 100',[]);return {entries:rows.map((r,i)=>({rank:i+1,uid:r.uid,distance:Number(r.distance),...r.snapshot,achieved:r.achieved_at}))};}
  if(action==='register'){
  ...
  if(action==='start'){
   ...
   const existing=await query('SELECT id,uid FROM mm_voyages WHERE id=$1',[b.runId]);if(existing.length){if(existing[0].uid!==uid)fail('AUTH',403);return {runId:b.runId,protocol:1};}
   ...
   const snapshot={name:flightName(b.name),design:flightDesign(b.design)};
   ...
   if(!inserted.length){const rows=await query('SELECT uid FROM mm_voyages WHERE id=$1',[b.runId]);if(rows[0]?.uid!==uid)fail('AUTH',403);}
   return {runId:b.runId,protocol:1};
  }
  ...
  if(action==='goal'){
   ...
   if(b.seq!==run.steps+1||!levelOK||b.target!==voyageStage(b.level).target)return invalid();
   const updated=await query("UPDATE mm_voyages SET steps=steps+1,last_level=$3,distance=distance+$4,updated_at=now() WHERE id=$1 AND uid=$2 AND state='open' AND steps=$5 RETURNING steps",[b.runId,uid,b.level,voyageStage(b.level).target,run.steps]);
  ...
  if(action==='finish'){
   ...
   await query(`WITH finished AS (UPDATE mm_voyages SET state='finished',updated_at=now() WHERE id=$1 AND uid=$2 AND state='open' AND steps=$3 AND distance=$4 RETURNING uid,distance,snapshot)
    INSERT INTO mm_bests(uid,distance,snapshot) SELECT uid,distance,snapshot FROM finished WHERE distance>0
    ON CONFLICT(uid) DO UPDATE SET distance=EXCLUDED.distance,snapshot=EXCLUDED.snapshot,achieved_at=now() WHERE EXCLUDED.distance>mm_bests.distance`,[b.runId,uid,b.count,b.distance]);
